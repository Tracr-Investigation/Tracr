...
async def list_selector_types(user=Depends(get_current_user)):
    """Referentiel des types de selecteurs (pour alimenter les listes du front)."""
    return {
        "types": [
...
async def list_hits(
    ...
    """Renvoie les correspondances DEJA enregistrees (dernier scan) avec la date
    de derniere analyse. Lecture seule, sans recalcul."""
    _check_investigation_access(db, investigation_id, user.id_user)
...
async def scan_hits(
    ...
    """(Re)lance l'analyse : confronte les selecteurs au texte des sources,
    enregistre et renvoie les correspondances."""
    _check_investigation_access(db, investigation_id, user.id_user)
