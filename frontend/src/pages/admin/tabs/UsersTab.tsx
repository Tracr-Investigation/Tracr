          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left px-6 py-4 text-sm font-medium text-text-muted">Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="px-6 py-12 text-center text-text-muted">{t('admin.users.loading')}</td></tr>
              ) : users.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-12 text-center text-text-muted">{t('admin.users.empty')}</td></tr>
              ) : (
                users.map((u) => (
            ...
            <div className="flex items-center gap-1">
              ...
              {getPageNumbers().map((p) => (
                <button key={p} onClick={() => setPage(p)}
                  className={`w-9 h-9 rounded-lg text-sm font-medium transition-all ${p === page ? 'bg-primary/20 text-text-default border border-primary/30' : 'bg-card text-text-muted hover:bg-primary/10 hover:text-text-default'}`}>
                  {p}
                </button>
              ))}
